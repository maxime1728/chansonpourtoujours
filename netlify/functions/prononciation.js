...
const MG_FROM    = process.env.MAILGUN_FROM || 'Chanson Pour Toujours <info@chansonpourtoujours.ca>';
const TEAM_EMAIL = process.env.TEAM_NOTIFY_EMAIL;   // destinataire de l'alerte interne
const SITE       = 'https://chansonpourtoujours.ca';
...
const SYSTEM = `Tu aides l'équipe de Chanson Pour Toujours à corriger la PRONONCIATION d'un mot qu'un chanteur IA prononce mal dans une chanson (le plus souvent en français québécois). Le client décrit le mot et comment il devrait sonner ; tu produis une correction prête à utiliser.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (mot) try {
      const userPrompt =
`Personne honorée : ${p.recipient_name || ''}

...
function systemCreate(lang) {
  return `You are a professional songwriter for Chanson Pour Toujours, a service that creates personalized tribute songs for someone who has passed away.
...
function systemRegenerate(lang, isGift) {
  return `You are a professional songwriter for Chanson Pour Toujours, revising existing ${isGift ? 'gift' : 'tribute'} lyrics at the client's request.
...
function systemCreateGift(lang) {
  return `You are a professional songwriter for Chanson Pour Toujours, creating a personalized GIFT song to celebrate a LIVING person (birthday, love, just because).
...
- In memory of: ${p.recipient_name || 'cette personne'}
- Relationship to the person ordering: ${p.relationship || ''}
- Musical style: ${p.music_style || ''}
- Mood: ${p.mood || ''}
- What made them unique: ${p.what_makes_special || ''}
...
- What we want to keep and pass on: ${p.special_phrase || ''}`;
      ...
      const titre = parsed.title || `Pour ${p.recipient_name || 'cette personne'}`;
...
- In memory of: ${p.recipient_name || ''}
- Relationship: ${p.relationship || ''}
- Musical style: ${p.music_style || ''}
- Mood: ${p.mood || ''}
- What made them unique: ${p.what_makes_special || ''}
- Shared memories: ${p.memories || ''}
- What to keep and pass on: ${p.special_phrase || ''}
...
${modifications}`;
      ...
      const nouveauTitre = titreActuel || parsed.title || `Pour ${p.recipient_name || 'cette personne'}`;
  ...
  const recipient_name    = d.recipient_name    || 'cette personne';
  const relationship     = d.relationship     || '';
  const music_style      = d.music_style      || 'douce mélodie';
  const mood             = d.mood             || '';
  const what_makes_special = d.what_makes_special || '';
  const memories         = d.memories         || '';
  const special_phrase   = d.special_phrase   || '';

  const userPrompt =
`Provided information:
- In memory of: ${recipient_name}
- Relationship to the person ordering: ${relationship}
- Musical style: ${music_style}
- Mood: ${mood}
- What made them unique: ${what_makes_special}
...
- What we want to keep and pass on: ${special_phrase}`;
  ...
  try {
    ...
    return {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        title:       parsed.title || `Pour ${recipient_name}`,
        lyrics:      parsed.lyrics,
